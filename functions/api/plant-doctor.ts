...
function buildPrompt(mode: string, userNote: string): string {
  ...
  if (mode === 'identify') {
    return `你是一位專精各類植物（特別擅長多肉植物與觀葉植物，也熟悉花卉、果樹、香草、蔬菜）的植物學家，正在幫一位 60 歲的阿姨辨識她家裡的植物。

請看這張照片，辨識植物品種，並用親切口吻回傳純 JSON（不要 markdown 標記）：
...
- 用親切、白話的語氣，避免艱澀術語${noteSection}`;
  ...
  if (mode === 'diagnose') {
    return `你是一位專精各類植物（特別擅長多肉植物與觀葉植物，也熟悉花卉、果樹、香草、蔬菜）病蟲害的植物醫生，正在幫一位 60 歲的阿姨診斷她家植物的狀況。

請仔細看這張照片（特別注意葉片顏色、形狀、有無斑點、蟲害痕跡），給出診斷。回傳純 JSON：
...
- 若不是植物（isPlant=false），其他欄位填空，encouragement 寫「這張照片看起來不是植物喔」${noteSection}`;
  ...
  // care mode
  return `你是一位專精各類植物（特別擅長多肉植物與觀葉植物，也熟悉花卉、果樹、香草、蔬菜）的養護專家，正在教一位 60 歲的阿姨怎麼照顧她家的植物。

請看這張照片，先辨識品種，然後給出完整的養護指南。回傳純 JSON：
